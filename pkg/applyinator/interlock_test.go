...
func TestCheckInterlockWriteActiveSuccess(t *testing.T) {
	t.Parallel()
	...
	if err != nil {
		t.Fatalf("expected checkInterlock to succeed, got %v", err)
	}
	defer cleanup()
	...
	if err != nil {
		t.Fatalf("failed to read active interlock file: %v", err)
	}

	owner, ok := parseInterlockOwner(content)
	if !ok {
		t.Fatalf("expected valid interlock owner content, got %q", string(content))
	}
	if owner.PID != os.Getpid() {
		t.Errorf("expected PID %d, got %d", os.Getpid(), owner.PID)
	}
}

func TestCheckInterlockWriteActiveError(t *testing.T) {
	if runtime.GOOS == "windows" || os.Geteuid() == 0 {
		t.Skip("permission checks require non-root execution on POSIX systems")
	}
	t.Parallel()

	// Create a read-only directory to force a write error
	interlockDir := filepath.Join(t.TempDir(), "readonly-interlock")
	if err := os.MkdirAll(interlockDir, 0500); err != nil {
		t.Fatal(err)
	}
	...
	if err == nil {
		...
		t.Fatal("expected error writing active interlock file in read-only directory, got nil")
	}
}
