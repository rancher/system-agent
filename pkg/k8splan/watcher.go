	...
	core.Secret().OnChange(ctx, "secret-watch", func(_ string, secret *v1.Secret) (*v1.Secret, error) {
		...
		if planData, ok := secret.Data[planKey]; ok {
			...
			if !hasRunOnce {
				...
				for _, instruction := range cp.Plan.OneTimeInstructions {
					if instruction.ResetFailureCountOnServiceRestart {
						logrus.Debugf("Resetting failure count due to system agent restart")
